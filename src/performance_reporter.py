...
def generate_report():
    ...
    # Ambil sinyal yang sudah ditutup
    cursor.execute('''
    ...
    # Buat format laporan Markdown
    report_lines = [
        f"# 📊 Laporan Kinerja Metode Bot XAUUSD",
    ...
    tg_lines = [
        "📊 **UPDATE KINERJA SEMUA METODE** 📊\n"
    ]
    
    # Sort by total trades descending, then alphabetically
    ...
    for method_name, stats in sorted_methods:
        ...
        # Tambahkan ke Telegram semuanya
        emoji = "⚪" if total == 0 else "🟢" if win_rate >= 60 else "🟠" if win_rate >= 40 else "🔴"
        tg_lines.append(f"{emoji} **{method_name}**")
        tg_lines.append(f"└ Win: {win} | Loss: {loss} | WR: {win_rate:.1f}%\n")
    ...
    # Kirim ke Telegram (batas panjang pesan TG)
    tg_message = "\n".join(tg_lines)
    if len(tg_message) > 4000:
        tg_message = tg_message[:3900] + "\n... (Laporan terlalu panjang, lihat file .md)"

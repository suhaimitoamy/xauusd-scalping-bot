...
def simulate(df: pd.DataFrame, expiry_bars: int = 60, max_hold_bars: int = 240,
             max_layers: int = 3, layer_spacing: float = 2.0, sl_buffer: float = 4.0,
             tp1_dist: float = 1.5, tp2_dist: float = 3.0) -> List[Dict]:
    ...
    while i < n - expiry_bars - max_hold_bars - 1:
        ...
        layer_prices = [pp + ((-layer_spacing * x) if d == 'BUY' else (layer_spacing * x)) for x in range(max_layers)]
        filled = [layer_prices[0]]
        next_layer = 1
        avg = filled[0]
        sl = (layer_prices[-1] - sl_buffer) if d == 'BUY' else (layer_prices[-1] + sl_buffer)
        tp1 = avg + tp1_dist if d == 'BUY' else avg - tp1_dist
        tp2 = avg + tp2_dist if d == 'BUY' else avg - tp2_dist
        status = 'ACTIVE'
        out = None
        out_i = None
        max_layer_used = 1
        ...
        for k in range(trigger, min(trigger + max_hold_bars, n)):
            ...
            if d == 'BUY':
                while status == 'ACTIVE' and next_layer < max_layers and lo <= layer_prices[next_layer]:
                    filled.append(layer_prices[next_layer])
                    next_layer += 1
                    max_layer_used = len(filled)
                ...
                while status == 'ACTIVE' and next_layer < max_layers and hi >= layer_prices[next_layer]:
                    filled.append(layer_prices[next_layer])
                    next_layer += 1
                    max_layer_used = len(filled)
                    avg = sum(filled) / len(filled)
        ...
        if out:
            trades.append({
                **sig,
                'pending_price': round(pp, 3),
                'average_entry': round(avg, 3),
                'layers_used': max_layer_used,
                'result': out,
                'signal_time': rows[i].Index,
                'entry_time': rows[trigger].Index,
                'exit_time': rows[out_i].Index,
            })
            next_i = out_i + 1
            i = out_i + 1
        else:
...
def summarize(paths: List[str], title: str):
    ...
    for path in paths:
        ...
        l2 = sum(t.get('layers_used', 1) >= 2 for t in trades)
        l3 = sum(t.get('layers_used', 1) >= 3 for t in trades)
        avg_layers = (sum(t.get('layers_used', 1) for t in trades) / len(trades)) if trades else 0
        label = Path(path).stem.split('_')[-1]
        lines.append(f"{label}: {len(trades)} trades, {wins}W / {losses}L, WR {wr:.2f}%, avg layers {avg_layers:.2f}, L2+ {l2}, L3 {l3}")
        total.extend(trades)
    wins = sum(t['result'] != 'LOSS' for t in total)
    losses = sum(t['result'] == 'LOSS' for t in total)
    wr = (wins / (wins + losses) * 100) if wins + losses else 0
    l2 = sum(t.get('layers_used', 1) >= 2 for t in total)
    l3 = sum(t.get('layers_used', 1) >= 3 for t in total)
    avg_layers = (sum(t.get('layers_used', 1) for t in total) / len(total)) if total else 0
    lines.append(f"TOTAL: {len(total)} trades, {wins}W / {losses}L, WR {wr:.2f}%, avg layers {avg_layers:.2f}, L2+ {l2}, L3 {l3}")
    return lines
...
def main():
    ...
    if not paths:
        ...
        return
    print('\n'.join(summarize(paths, 'CONTROLLED AVERAGING BACKTEST')))
